...
echo ""
echo "*******************************************************"
echo "** Sylius Mollie IMAGE"
echo "*******************************************************"
echo ""
echo "launching docker...please wait..."
echo ""
...
echo "MOLLIE: setting timezone to ${TZ}..."
...
if [ $SSH_USER != "not-set" ] && (! id -u "${SSH_USER}" >/dev/null 2>&1 ); then
    echo "MOLLIE: creating additional SSH user...."
    ...
    sudo usermod -s /bin/false mollie
    # allow ssh in sshd_config
    sudo sed -i "s/AllowUsers mollie/AllowUsers ${SSH_USER}/g" /etc/ssh/sshd_config
    echo "-----------------------------------------------------------"
fi

# start the SSH service with the latest setup
echo "MOLLIE: restarting SSH service...."
sudo service ssh restart
echo "-----------------------------------------------------------"

echo "MOLLIE: starting MySQL...."
...
if [ $MYSQL_USER != "not-set" ] && [ $MYSQL_PWD != "not-set" ]; then
    echo "MOLLIE: creating new MySQL user...."
    # -----------------------------------
...
echo "MOLLIE: starting mailcatcher...."
sudo /usr/bin/env $(which mailcatcher) --ip=0.0.0.0
echo "-----------------------------------------------------------"

echo "MOLLIE: starting cron service...."
...
sudo sed -i 's#__mollie_apache_docroot__#'${APACHE_DOCROOT}'#g' /etc/apache2/sites-enabled/000-default.conf
# --------------------------------------------------

echo "MOLLIE: switching to PHP ${PHP_VERSION}..."
cd /var/www && make switch-php version=${PHP_VERSION}
sudo service apache2 stop
echo "-----------------------------------------------------------"

if [ $COMPOSER_VERSION = 1 ]; then
   echo "MOLLIE: switching to composer 1..."
   sudo composer self-update --1
   echo "-----------------------------------------------------------"
fi
if [ $COMPOSER_VERSION = 2 ]; then
   echo "MOLLIE: switching to composer 2..."
...
if [[ ! -z "$NODE_VERSION" ]]; then
   echo "MOLLIE: switching to Node ${NODE_VERSION}..."
...
echo "MOLLIE: testing and starting Apache..."
sudo apache2ctl configtest
sudo service apache2 restart
echo "-----------------------------------------------------------"

echo "MOLLIE: Restarting dbus..."
